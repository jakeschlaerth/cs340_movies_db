    ...
    req.onload = (e) => {
        if (req.readyState === 4) {
            if (req.status === 200) {
                ...
                alert(`Added director ${payload.first_name} ${payload.last_name}`)
            } else {
                console.error(req.statusText);
            }
        }
        ...
        req.onload = (e) => {
            if (req.readyState === 4) {
                if (req.status === 200) {
                    ...
                    updateBool = false;
                    // return success
                    alert(`updated director to ${payload.first_name} ${payload.last_name}`);
                } else {
                    console.error(req.statusText);
                }
            }
        ...
        if (req.readyState === 4) {
            if (req.status === 200) {
                ...
                for (i = 0; i < movies.length; i++) {
                    if (movies[i].director_id == searchID) {
                        // make a row for this movie
                        results.push(movies[i]);
                    }
                }
                if (results.length == 0) {
                    alert("Sorry, no movies with that director.")
                }
...
searchButton.addEventListener('click', (e) => {
    ...
    req.onload = (e) => {
        if (req.readyState === 4) {
            if (req.status === 200) {
                var response = JSON.parse(req.responseText);
                var allDirectors = response.rows
                // check if director exists
                var j;
                var exists = false;
                for (j = 0; j < allDirectors.length; j++) {
                    if (`${allDirectors[j].first_name.toLowerCase()} ${allDirectors[j].last_name.toLowerCase()}` == searchInput.value.toLowerCase()) {
                        exists = true;
                    }
                }
                if (exists) {
                    ...
                    new_req.setRequestHeader("table_name", "movies", false);    // set what table we are requesting
                    new_req.onload = (e) => {
                        if (new_req.readyState === 4) {
                            if (new_req.status === 200) {
                                ...
                                var results = [];
                                for (i = 0; i < movies.length; i++) {
                                    if (movies[i].director.toLowerCase() == searchInput.value.toLowerCase()) {
                                        // make a row for this movie
                                        results.push(movies[i]);
                                    }
                                }
